...
const faseColors: Record<string, string> = {
  'Conhecimento': T.copper,
  'Recursal':     '#9080C8',
  'Execução':     '#C07070',
  'Liquidação':   '#8090B0',
}
const riscoText: Record<string, string> = {
  'Provável': T.red,
  'Possível': T.amber,
  'Remoto':   T.green,
}
  ...
  return (
    <div className="p-6" style={{ background: T.s1, border: `1px solid ${T.b1}` }}>
      <p className="text-xs tracking-widest uppercase mb-5"
        style={{ color: T.t3, letterSpacing: '0.12em' }}>{label}</p>
...
export default function Dashboard() {
  ...
  const prazos     = processosMock.flatMap(p => p.prazos).filter(p => p.status === 'Pendente')
  ...
  const faseData = ['Conhecimento', 'Recursal', 'Execução', 'Liquidação']
  ...
  const evolucao = [
    { m: 'Jan', pedidos: 620, risco: 250 },
    { m: 'Fev', pedidos: 650, risco: 268 },
    { m: 'Mar', pedidos: 720, risco: 290 },
    { m: 'Abr', pedidos: 890, risco: 380 },
    { m: 'Mai', pedidos: 1060, risco: 481 },
  ]
    ...
    <div>
      ...
      <div className="p-8 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <KpiCard label="Prazos Pendentes"  value={prazos.length}     sub="a vencer" delta="requer atenção" />
          <KpiCard label="Audiências"        value={audiencias.length} sub="agendadas" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="p-6" style={{ background: T.s1, border: `1px solid ${T.b1}` }}>
            <p className="text-xs tracking-widest uppercase mb-3" style={{ color: T.t3, letterSpacing: '0.12em' }}>
              Total em Pedidos
            </p>
            <p className="text-3xl font-light" style={{ color: T.t1, letterSpacing: '-0.5px' }}>
              {formatCurrency(totalPedido)}
            </p>
            <p className="text-xs mt-2" style={{ color: T.t3 }}>soma dos pedidos dos reclamantes</p>
          </div>
          <div className="p-6" style={{ background: T.s1, border: `1px solid ${T.b1}` }}>
            <p className="text-xs tracking-widest uppercase mb-3" style={{ color: T.t3, letterSpacing: '0.12em' }}>
              Risco Estimado SCC
            </p>
            <p className="text-3xl font-light" style={{ color: T.copper, letterSpacing: '-0.5px' }}>
              {formatCurrency(totalRisco)}
            </p>
            <p className="text-xs mt-2" style={{ color: T.t3 }}>
              {((totalRisco / totalPedido) * 100).toFixed(0)}% dos pedidos — exposição estimada
            </p>
          </div>
        </div>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={faseData} cx="50%" cy="50%" innerRadius={48} outerRadius={72}
                  ...
                  {faseData.map(d => <Cell key={d.name} fill={faseColors[d.name] || T.copper} />)}
                </Pie>
              ...
              {faseData.map(d => (
                <div key={d.name} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ background: faseColors[d.name] || T.copper }} />
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={riscoBarData} barSize={32}>
                ...
                <Bar dataKey="value" name="Processos" radius={[3, 3, 0, 0]}>
                  {riscoBarData.map(d => <Cell key={d.name} fill={riscoBg[d.name]} stroke={riscoText[d.name]} strokeWidth={1} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            <div className="flex gap-4 mt-3">
              {riscoBarData.map(d => (
                <div key={d.name} className="flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: riscoText[d.name] }} />
                  <span className="text-xs" style={{ color: T.t2 }}>
                    {d.name.slice(0, 1)}: <strong style={{ color: T.t1 }}>{d.value}</strong>
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="p-6" style={{ background: T.s1, border: `1px solid ${T.b1}` }}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={evolucao}>
                <defs>
                  <linearGradient id="gPedido" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis tick={{ fontSize: 11, fill: T.t3 }} axisLine={false} tickLine={false} />
                <Tooltip content={<CustomTooltip />} />
                <Area type="monotone" dataKey="pedidos" name="Pedidos" stroke={T.copper} strokeWidth={2} fill="url(#gPedido)" />
                <Area type="monotone" dataKey="risco"   name="Risco"   stroke={T.red}    strokeWidth={2} fill="url(#gRisco)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          <div style={{ background: T.s1, border: `1px solid ${T.b1}` }}>
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: `1px solid ${T.b1}` }}>
              <p className="text-xs tracking-widest uppercase" style={{ color: T.t3, letterSpacing: '0.12em' }}>
                Próximos Eventos
              </p>
          ...
          <div style={{ background: T.s1, border: `1px solid ${T.b1}` }}>
            ...
            {processosMock
              ...
              .sort((a, b) => b.verbas.riscoSCC - a.verbas.riscoSCC)
              .slice(0, 6)
              .map((p, i, arr) => (
                <div key={p.id} onClick={() => navigate(`/processos/${p.id}`)}
                  className="flex items-center justify-between px-6 py-3.5 cursor-pointer transition-colors"
                  ...
                  <div className="flex-1 min-w-0 mr-4">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span style={{ color: riscoText[p.risco], fontSize: 11 }}>· {p.risco}</span>
                    </div>
                  </div>
                  <p className="text-xs font-semibold flex-shrink-0" style={{ color: T.copper }}>
                    {formatCurrency(p.verbas.riscoSCC)}
                  </p>
                </div>
              ))}
          </div>
